...
public OnClientPostAdminCheck(client)
{
	if(GetConVarBool(sm_whitelist_enable) && !IsFakeClient(client) && !IsImmune(client))
	{
		...
		LogMessage("Validating AuthID: %s", auth);
		LogMessage("Amount of whitelisted : %s", listlen);
		new bool:allow = false;
		for(new i; i < listlen; i++)
		{
			LogMessage("%s is this %s ?", auth, whitelist[i]);
...
public LoadList()
{
	...
	{
		LogMessage("Validating this line : %s", steamid);
		if (steamid[0] == ';')
		{
			LogMessage("Is not char alpha");
			continue;
		}
		...
		LogMessage("Adding this steamid : %s", steamid);
		whitelist[listlen] = steamid;
		listlen++;
	}
